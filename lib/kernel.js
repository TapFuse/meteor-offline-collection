...
let _nextTimerReferenceId = 0;
const _timerRunning = {};

const _initTimer = function() {
  // Get id
  const id = _nextTimerReferenceId++;
...
Kernel.setInterval = function(f, interval) {
  ...
  // Calc the next run
  let nextRun = Kernel.now() + interval;

  // The interval function
  const intervalFunction = function intervalFunction() {
    if (_timerRunning[id]) {
      // Calc the next run
      nextRun += interval;
...
//   // new one established).
//   //
//   // We could lift these restrictions in letious ways.  One interesting
...
//   const c = Kernel.autorun(function viewAutorun(c) {
...
Kernel.loop = function renderLoop() {
  ...
  const currentTimedFunctions = timedFunctions;

  // Reset timedFunctions
  timedFunctions = [];

  for (let i = 0; i < currentTimedFunctions.length; i++) {
    const timedFunction = currentTimedFunctions[i];
  ...
  let renderLength = renderFunctions.length;
  ...
  let displayForcedDeferedCount = true;
  let displayDeferedCount = true;
