...
offLineCollection.Collection = class offLineCollectionClass extends Mongo.Collection {
  ...
  startCollection() {
    Tracker.autorun(() => {
      if (this.userPresent.get()) {
        _consoleLog('⚡️ startCollection', 'this.userPresent.get()', this.userPresent.get());
        if (this.session) {
          if (!this.onlySession) {
  ...
  subscribeData(subscriptionName) {
    _consoleLog('💣 called subscribeData', subscriptionName);
    if (subscriptionName) {
      _consoleLog('⚡️ subscribeData', 'subscriptionName', subscriptionName);
      if (this.session) {
        Tracker.autorun( c => {
        ...
        Tracker.autorun( c => {
          if (this.userPresent.get()) {
            ...
            this.subscribtionsHandler = Meteor.subscribe(subscriptionName, {lastUpdatedAt: this.lastUpdatedAt ? this.lastUpdatedAt : 1});
          } else {
            c.stop();
          }
          if (!this.isLoaded.get()) {
  ...
  monitorExternalChanges(externalCollection, observeRemove) {
    ...
    Tracker.autorun( c => {
      ...
      Collection[externalCollection].find(query).observe({
        added: doc => {
          if (!doc.isDeleted) {
            if (this.removalField && doc[this.removalField]) {
  ...
  monitorChanges() {
    ...
    Tracker.autorun(() => {
      if (this.isLoaded.get()) {
        _consoleLog('⚡️ monitorChanges', 'this.isLoaded.get()');
        const query = {};
        if (this.session) {
          if (this.activeSession.get()) {
            query[this.sessionField] = this.activeSession.get();
