...
// alert(`${localforage.supports(localforage.WEBSQL)}`)
// localforage.setDriver([localforage.WEBSQL])
const localPath = 'localhost:12832';
// localforage.setDriver([localforage.WEBSQL])
...
function _getFile(filePath) {
  ...
  return fetch(`http://${localPath}/data/${filePath}`)
    .then(response => {
...
async function _updateServerSettings() {
  try {
    ...
    // console.log('🎨', newData);
    if (newData) {
      const dbRemoved = await _removeLocalStorage(
        'offlineServerSettings',
      );
      if (dbRemoved) {
        // console.log('🎨 2');
        const dbUpdated = await updateLocalStorage(
          'offlineServerSettings',
          newData.data,
        );
...
async function _startMeteorUsersUpdate() {
  try {
    const newData = await _getFile('MeteorUsers.json');
    // console.log('🎨', newData);
    if (newData) {
      const dbRemoved = await _removeLocalStorage(
        'offlineMeteorUsers',
    ...
    console.log('🌶 _startMeteorUsersUpdate', err);
  }
}
...
async function _checkWhichEventsToRemove(eventIdArray) {
  try {
    const storedEvents = await _getLocalStorageItemIds(
      'offlineSingleEvents',
    );
    const eventsToRemove = _difference(
      storedEvents,
      eventIdArray,
    );
    for (const item of eventsToRemove) {
      ...
      for (const eventInfo of collectionList) {
        await _removeLocalStorage(
          `offline${collectionName}${eventInfo}`,
        );
      }
    }
    return 'done';
...
async function _checkWhichEventsToUpdate(eventIdArray) {
  try {
    for (const eventInfo of eventIdArray) {
      const localTimeSyncedAt = (await localStorage.getItem(
        `SingleEvents${eventInfo._id}_lastSynced`,
      )) || 0;
      // console.log('🖕 1', localTimeSyncedAt);
      const localTimeUpdatedAt = (await localStorage.getItem(
        `SingleEvents${eventInfo._id}_updatedAt`,
      )) || 0;
      // check if update all Event collections
      if (
        _shouldUpdate(
          localTimeSyncedAt,
          eventInfo.lastSynced,
        )
      ) {
        const eventData = await _getFile(
          'SingleEvents.json',
        );
        if (eventData) {
          ...
          await updateLocalStorage(
            'offlineSingleEvents',
            eventData.dataList.SingleEvents,
          );
          const reSofflineImages = await _removeLocalStorage(
            'offlineImages',
          );
          if (reSofflineImages) {
            await updateLocalStorage(
              'offlineImages',
              eventData.dataList.Images,
            );
          }
          const reSofflineEventCategories = await _removeLocalStorage(
            'offlineEventCategories',
          );
          if (reSofflineEventCategories) {
            await updateLocalStorage(
              'offlineEventCategories',
              eventData.dataList.EventCategories,
            );
          }
          const reSofflineEventTags = await _removeLocalStorage(
            'offlineEventTags',
          );
          if (reSofflineEventTags) {
            await updateLocalStorage(
              'offlineEventTags',
              eventData.dataList.EventTags,
            );
          }
          const reSofflineEventDivisions = await _removeLocalStorage(
            'offlineEventDivisions',
          );
          if (reSofflineEventDivisions) {
            await updateLocalStorage(
              'offlineEventDivisions',
              eventData.dataList.EventDivisions,
            );
          }
          const reSofflineUserGroups = await _removeLocalStorage(
            'offlineUserGroups',
          );
          if (reSofflineUserGroups) {
            await updateLocalStorage(
              'offlineUserGroups',
              eventData.dataList.UserGroups,
            );
          }
          const reSofflineMenuItems = await _removeLocalStorage(
            'offlineMenuItems',
          );
          if (reSofflineMenuItems) {
            await updateLocalStorage(
              'offlineMenuItems',
              eventData.dataList.MenuItems,
            );
          }

          const reSofflineSponsors = await _removeLocalStorage(
            'offlineSponsors',
          );
          if (reSofflineSponsors) {
            await updateLocalStorage(
              'offlineSponsors',
              eventData.dataList.Sponsors,
            );
          }
          const reSofflineSponsorsCategory = await _removeLocalStorage(
            'offlineSponsorsCategory',
          );
          if (reSofflineSponsorsCategory) {
            await updateLocalStorage(
              'offlineSponsorsCategory',
              eventData.dataList.SponsorsCategory,
            );
          }
        }
      }
      ...
      if (
        ...
        for (const collectionName of collectionList) {
          const collectionData = await _getFile(
            `events/${eventInfo._id}_${collectionName}.json`,
          );
          if (collectionData) {
            // await _removeLocalStorage(`offline${collectionName}`);
            const resultRemove = await _removeLocalStorage(
              `offline${collectionName}${eventInfo._id}`,
            );
            if (resultRemove) {
              await updateLocalStorage(
                `offline${collectionName}${eventInfo._id}`,
                collectionData.dataList,
              );
            }
          }
        }
...
async function _startAppUpdate() {
  try {
    const eventsArray = dataHandler.get(
      'syncedData',
    ).events;
    ...
    if (eventsArray.length > 0) {
      ...
      const resultRemove = await _checkWhichEventsToRemove(
        eventsArray,
      );
      const result = await _checkWhichEventsToUpdate(
        eventsArray,
      );
      ...
      console.log(
        '❌ - _startAppUpdate, array is empty',
        eventsArray,
      );
      dataHandler.set('eventsReady', true);
    }
...
function _checkIfLocalDataIsNewer(
  serverSyncedAt,
  serverUpdatedAt,
  bundleData,
) {
...
HTTP.get(
  `http://${localPath}/data/lastSynced.json`,
  {},
  (error, result) => {
    if (result) {
      const appVersion = 1;
      ...
      // const lastVersion = localStorage.getItem('appVersion') || 0;
      // if (appVersion === lastVersion) {
      const serverSyncedAt = localStorage.getItem(
        'ServerSettings_lastSynced',
      ) || 0;
      const serverUpdatedAt = localStorage.getItem(
        'ServerSettings_updatedAt',
      ) || 0;
      // } else {
      //   // localStorage.clear();
      ...
      const bundleData = result.data && result.data.server
        ? result.data.server
        : 0;
      if (!bundleData) {
        console.log(
          '❌',
          'no bundle data was found',
          result.data,
        );
      }
      _checkIfLocalDataIsNewer(
        serverSyncedAt,
        serverUpdatedAt,
        bundleData,
      );
...
Tracker.autorun(c => {
  if (
    dataHandler.get('serverReady') &&
    dataHandler.get('eventsReady')
  ) {
    dataHandler.set('appIsReady', true);
    c.stop();
  }
