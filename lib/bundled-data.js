...
// localforage.setDriver([localforage.WEBSQL])
const collectionList = ['SessionProgramme', 'Sessions', 'Tickets', 'CompanyTickets', 'Sponsors', 'SponsorsCategory', 'InfoPage', 'Maps', 'Tags', 'MapLocations', 'MapLocationsCategory', 'FileLinks'];
...
async function _startMeteorUsersUpdate() {
  try {
    ...
    // console.log('🎨', newData);
    if (newData) {
      const dbRemoved = await _removeLocalStorage('offlineMeteorUsers');
      if (dbRemoved) {
        // console.log('🎨 2');
        const dbUpdated = await updateLocalStorage('offlineMeteorUsers', newData.data);
        // console.log('🎨 4', dbUpdated);
        if (await dbUpdated) {
          // console.log('🎨 3');
          dataHandler.set('serverReady', true);
...
async function _checkWhichEventsToUpdate(eventIdArray) {
  try {
    for (const eventInfo of eventIdArray) {
      const localTimeSyncedAt = await localStorage.getItem(`SingleEvents${eventInfo._id}_lastSynced`) || 0;
...
HTTP.get(`http://${localPath}/data/lastSynced.json`, {},
  (error, result) => {
    if (result) {
      const appVersion = 1;
      ...
      const serverSyncedAt = localStorage.getItem('ServerSettings_lastSynced') || 0;
